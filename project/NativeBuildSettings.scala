import Utils.{executeProcess, getProcessLogger, nativeRoot}
import sbt.*
import sbt.Keys.*
        ...
        Def.task {
          ...
          sys.env.get("SKIP_NATIVE_GENERATION") match {
            case None =>
              ...
              val cargoTomlPath = s"${baseDirectory.value}/native/Cargo.toml"  //"D:\\data\\git\\storch-polars\\native\\Cargo.toml"   s"${baseDirectory.value.getParent}/native/Cargo.toml"
              println(s"\t\r rust cargo toml path : ${cargoTomlPath} || , baseDirectory.value ${baseDirectory.value} || baseDirectory.value.getParent ${baseDirectory.value.getParent} || nativeOutputDir：  ${nativeOutputDir} please check path is correct or not \r\t")
